...
(async () => {
    ...
    while (Date.now() - startTime < MAX_WAIT_MS + 30000) {
        ...
        for (const p of pages) {
            ...
            try {
                // SCROLLING LOGIC: Scroll down to reveal lazy buttons (Crucial for APKDone)
                await p.evaluate(() => {
                    window.scrollBy(0, 500);
                    if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight) {
                         window.scrollTo(0, 0); // Reset if bottom reached to check top again next loop
                    }
                });
                ...
                const decision = await p.evaluate(() => {
                    ...
                    for (let el of buttons) {
                        ...
                        if (text.includes('fast download')) score -= 200; // Penalize "Fast Download" wrapper

                        // ✅ GOOD WORDS
                        if (text === 'download') score += 100;
                        if (text === 'download apk') score += 150; // Higher priority
                        if (text.includes('download apk') && text.includes('mb')) score += 120; // Specific Target
